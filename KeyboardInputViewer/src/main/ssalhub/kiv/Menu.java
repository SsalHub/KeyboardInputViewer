...
import main.ssalhub.kiv.panels.AvgPanel;
import main.ssalhub.kiv.panels.TotPanel;
import main.ssalhub.kiv.ui.dialog.KeysDialog;
import main.ssalhub.kiv.ui.dialog.LayoutDialog;
...
public class Menu {
    ...
    protected static final void createMenu() {
        ...
        snap.addActionListener((e) -> {
            ...
            frame.setLocation(
                    Math.abs(loc.x - bounds.x) < 100 ? bounds.x
                            : Math.abs((loc.x + frame.getWidth()) - (bounds.x + bounds.width)) < 100
                                    ? bounds.x + bounds.width - frame.getWidth()
                                    : loc.x,
                    Math.abs(loc.y - bounds.y) < 100 ? bounds.y
                            : Math.abs((loc.y + frame.getHeight()) - (bounds.y + bounds.height)) < 100
                                    ? bounds.y + bounds.height - frame.getHeight()
                                    : loc.y);
        });
    ...
    private static final class MenuUI extends BasicMenuUI implements MouseListener, PopupMenuListener {
        ...
        @Override
        public Dimension getPreferredMenuItemSize(JComponent c, Icon checkIcon, Icon arrowIcon,
        ...
        @Override
        public void paintMenuItem(Graphics g, JComponent c, Icon checkIcon, Icon arrowIcon, Color bg, Color fg,
                int defaultTextIconGap) {
         ...
         */
        private static final void paintMenuItem(Graphics2D g, JMenuItem menuItem, boolean hasCursor,
                int defaultTextIconGap) {
            ...
            if (menuItem instanceof JCheckBoxMenuItem && menuItem.isSelected()) {
                ...
                g.drawImage(ColorManager.arrow, menuItem.getWidth() - 12, 5, menuItem.getWidth(), 17, 0, 0, 128, 128,
                        menuItem);
            }
