...
public class VersionControl {
	...
	public static final String getLatestVersion(String repository){
		try {
			HttpURLConnection con = (HttpURLConnection)new URL("https://api.github.com/repos/SsalHub/" + repository + "/tags").openConnection();
	...
	public static final JLabel getVersionLabel(String repos, String currentVer) {
		JLabel ver = new JLabel(String.format(VERSION_FORMAT_ITALICS, currentVer, "<i><font color=gray>loading</font></i>"), SwingConstants.CENTER);
		
		
		new Thread(()->{
			latestVer = getLatestVersion(repos);
			ver.setText(String.format(VERSION_FORMAT_ITALICS, currentVer, latestVer == null ? "unknown :(" : latestVer));
	...
	public static final void autoUpdate(String repos, String currentVer) {
		...
		if(cur_main > max_main || cur_sub >= max_sub) return;
