...
public class ColorManager {
    ...
    public static final void prepareImages(boolean custom) throws IOException {
        if (custom) {
            ...
            alphaAqua = new Color(Main.config.getForegroundColor().getRed(),
                    Main.config.getForegroundColor().getGreen(), Main.config.getForegroundColor().getBlue(), 51);
    ...
    private static final int repaintPixel(int original, Color newcolor) {
        return (((original >> 24) & 0x000000FF) << 24)
                | (((((original >> 16) & 0x000000FF) + 2 * newcolor.getRed()) / 3) << 16)
                | (((((original >> 8) & 0x000000FF) + 2 * newcolor.getGreen()) / 3) << 8)
                | (((original & 0x000000FF) + 2 * newcolor.getBlue()) / 3);
    }
}
