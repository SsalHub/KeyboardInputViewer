...
public class Util {
     ...
     */
    public static final JLabel getVersionLabel(String repository, String currentVersion, boolean italics,
            int alignment) {
     ...
     * @see #getVersionLabel(String, String)
     */
    public static final JLabel getVersionLabel(String user, String repository, String currentVersion, boolean italics,
            int alignment) {
     ...
     * @see #getVersionLabel(String, String)
     */
    public static final JLabel getVersionLabel(String user, String repository, String currentVersion, String def,
            boolean italics, int alignment) {
        JLabel ver = new JLabel(String.format(italics ? VERSION_FORMAT_ITALICS : VERSION_FORMAT, currentVersion,
                "<i><font color=gray>loading</font></i>"), alignment);
        checkVersion(user, repository, version -> ver.setText(
                String.format(italics ? VERSION_FORMAT_ITALICS : VERSION_FORMAT, currentVersion, version.orElse(def))));
        return ver;
    }
    ...
    public static final void checkVersion(String user, String repository, Consumer<Optional<String>> callback) {
        Thread thread = new Thread(() -> callback.accept(Optional.ofNullable(checkVersion(user, repository))),
                "Version Checker");
    ...
    public static final String checkVersion(String user, String repository) {
        try {
            HttpURLConnection con = (HttpURLConnection) new URL(
                    "https://api.github.com/repos/" + user + "/" + repository + "/tags").openConnection();
    ...
    public static final String readArtifactVersion(String group, String artifact) {
        try (InputStream pom = ClassLoader
                .getSystemResourceAsStream("META-INF/maven/" + group + "/" + artifact + "/pom.xml")) {
            if (pom == null) {
                return null;
            }
            ...
            NodeList data = docFactory.newDocumentBuilder().parse(pom).getElementsByTagName("project").item(0)
                    .getChildNodes();
